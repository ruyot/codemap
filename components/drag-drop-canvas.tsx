...
export default function DragDropCanvas({ 
  ...
  onFileSelect, 
  onFileUpload, 
  onFileContextMenu // <-- add this prop
}: DragDropCanvasProps) {
  ...
  const onNodeDoubleClick = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    if (node.data && typeof node.data.filePath === 'string' && node.data.type === 'file' && typeof onFileSelect === 'function') {
      const previewable = ['.tsx','.jsx','.html','.css'].some(ext => (node.data as any).filePath.endsWith(ext))
      onFileSelect(node.id, (node.data as any).filePath, previewable)
    }
  }, [onFileSelect])
